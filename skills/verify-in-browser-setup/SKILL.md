...
- **Data** — the seed or reset command, its guards, and what the seed leaves empty.
...
- **Gotchas** — only what no config confesses: the interface language a selector has to match, two controls whose labels prefix-collide, a screen that needs a hard reload. Facts the driver will hit and the repo never states.

Done when the file exists, is reachable from the pointer, and every persona row and the seed command ran in this session.
