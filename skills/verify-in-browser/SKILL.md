...
description: Walk a change through the running app in a browser, across every persona, and report what it actually does. The third axis of review, after a diff has been read against standards and spec. Use when the user asks to verify, QA, or smoke a change in the running app, or when a spec's behaviour has to be seen rather than read.
---
...
| Smoke, when the user asks for it and names the surface | the named surface, per persona | the obviously-broken bar |
...
Smoke has no per-case expected, so it runs against the **obviously-broken bar** instead: console errors, failed requests, 404s and 500s, blank regions where content should render, stuck loading states, unhandled errors, dead links, controls that render but do not respond.
...
Read the project's verify-in-browser setup file. `CLAUDE.md` or `AGENTS.md` names where the project keeps its agent docs; absent that, it is `.claude/verify-in-browser.md`.
